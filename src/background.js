        ...
        child.exec('reg query HKEY_CLASSES_ROOT\\*\\shell\\VSCode /v Icon', function(error, strOut, strError) {
            ...
            if (error) {
                ...
                if (os.platform !== 'win32') {
                    vshelp.showStatusBar('not win os,Please enter the path by yourself.(非win，请自己输入吧...)', 10000);
                } else {
...
function init(value, arr, noAction) { //逻辑入口
    ...
    return vshelp.prompt({
        ...
        placeHolder: "for example(比如)： E:\\Microsoft VS Code，/Users/YourID/.../VSCode.app"
    }).then(baseUrl => {
        if (!baseUrl) return;
